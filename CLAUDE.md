...
### 8. `confirm_with_diff` - Diff 付き確認

コード変更の差分を表示し、ユーザーに承認/否認を求める。

```
mcp__discord-approval__confirm_with_diff
├── message: string   # 変更内容の説明（必須）
├── diff: string      # 差分テキスト（必須、unified diff 形式推奨）
├── filename: string  # ファイル名（オプション、syntax highlight に使用）
└── timeout: number   # タイムアウト秒数（オプション、デフォルト300、最大900）
```
...
**注意事項:**
- 差分が 1500 文字を超える場合は自動的に切り詰められる
- ファイル名を指定すると、拡張子に応じた syntax highlight が適用される

### 9. `poll` - 複数選択投票

複数の選択肢から複数を選択してもらう場合に使用する。
...
**レスポンス:**
- 選択時: `ユーザーの選択 (2件):\n- 機能A\n- 機能C`
- 選択なし: `ユーザーは何も選択しませんでした`
- タイムアウト時: `タイムアウト: ユーザーからの応答がありませんでした`

**使用すべき場面:**
- 次回リリースに含める機能の選択
- 優先的に対応するバグの選択
- 複数のオプションから任意の組み合わせを選択

**`ask_question` との違い:**
| 項目 | ask_question | poll |
|------|-------------|------|
| 選択数 | 1件のみ | 複数可 |
| 用途 | 択一質問 | 複数選択投票 |
| 戻り値 | 単一文字列 | 文字列配列 |

### 10. `request_approval_with_reason` - 理由付き承認

承認/否認を求め、否認時に理由を入力してもらう。
...
- 承認時: `承認されました`
- 否認時: `否認されました\n理由: {ユーザー入力の理由}`
- 否認（理由なし）: `否認されました（理由なし）`
- タイムアウト時: `タイムアウト: ユーザーからの応答がありませんでした`

**使用すべき場面:**
- 否認理由を取得して改善に活かしたい場合
- 判断の根拠をログに残したい場合
- 次のアクションを決めるために理由が必要な場合

**`request_approval` との違い:**
| 項目 | request_approval | request_approval_with_reason |
|------|-----------------|------------------------------|
| 否認理由 | なし | Modal で取得 |
| UI | Button のみ | Button + Modal |
| 用途 | 単純な承認 | 理由が必要な承認 |

### 11. `create_thread` - スレッド作成

Discord にスレッドを作成する。長い作業の進捗をスレッドにまとめる場合に使用する。

```
mcp__discord-approval__create_thread
├── name: string     # スレッド名（必須、100文字以内）
└── message: string  # スレッドの最初のメッセージ（オプション）
```
...
- 長時間タスクの進捗をまとめたい場合
- 関連する通知をグループ化したい場合
- 後から参照しやすいように履歴を残したい場合

**注意事項:**
- スレッドは設定されたチャンネル内に作成される
- スレッド名は 100 文字以内
